...
class GameViewModel(
    ...
    fun updateProfileImage(id: String) {
        profileManager.updateProfileImage(id)
        saveData()
    }
    ...
    override var globalUpgrades =
        listOf(
            GlobalUpgrade(
                "double_click_10",
                Res.string.upg_precise_harvest_name,
                Res.string.upg_precise_harvest_desc,
                GamePrices.UPGRADE_PRECISE_HARVEST,
                5,
            ),
            GlobalUpgrade(
                "lucky_harvest",
                Res.string.upg_lucky_harvest_name,
                Res.string.upg_lucky_harvest_desc,
                GamePrices.UPGRADE_LUCKY_HARVEST,
                5,
            ),
            GlobalUpgrade(
                "weather_bonus",
                Res.string.upg_weather_bonus_name,
                Res.string.upg_weather_bonus_desc,
                GamePrices.UPGRADE_WEATHER_BONUS,
                1,
            ),
        )
    ...
    fun updateWeather(
        lat: Double,
        lon: Double,
    ) = weatherManager.updateWeather(lat, lon) { saveData() }
    ...
    private fun applySaveData(saveData: GameSaveData) {
        ...
        economyManager.applySaveData(
            saveData.totalClicks,
            saveData.money,
            saveData.totalMoneyEarned,
            saveData.fruitCounts,
            saveData.totalFruitHarvested,
        )
    ...
    private fun checkAchievements(isInitialLoad: Boolean = false) {
        achievements.forEach { achievement ->
            if (!unlockedAchievements.contains(achievement.id)) {
                val isEarned =
                    if (achievement.id == "art_collector") {
    ...
    fun uploadSaveToCloud(
        ...
        if (now - lastCloudSyncTime < 60000L) {
            onError(((60000L - (now - lastCloudSyncTime)) / 1000).toInt().toString())
            return
        }
        isCloudLoading = true
        viewModelScope.launch {
    ...
    fun downloadSaveFromCloud(
        ...
        viewModelScope.launch {
            try {
                ...
                if (jsonString != null) {
                    applySaveData(Json.decodeFromString<GameSaveData>(jsonString))
                    saveData()
                    onSuccess()
                } else {
                    onError("no_save")
    ...
    private fun getSaveDataSnapshot(): GameSaveData {
        ...
        return GameSaveData(
            ...
            username = username, profileImageId = profileImageId,
            unlockedAchievements = unlockedAchievements.toSet(),
            lastProfileUpdateTime = profileManager.lastProfileUpdateTime,
            fruitCounts = fruitCounts, totalFruitHarvested = totalFruitHarvested,
    ...
    fun selectItem(item: GameItem) {
        if (item.unlocked) currentItem = item
    }
    ...
    fun debugAddResources() {
        ...
        items.forEach {
            newCounts[it.id] = (newCounts[it.id] ?: 0) + 100000
            newTotals[it.id] = (newTotals[it.id] ?: 0) + 100000
        }
    ...
    fun resetGame() {
        ...
        profileManager.username = "Farmer"
        profileManager.profileImageId = "tomato"
        unlockedAchievements.clear()
        items =
            staticItemsList.mapIndexed {
                    i,
                    item,
                ->
                item.apply {
    ...
    private fun saveData() {
        viewModelScope.launch { gameRepository.saveGameData(getSaveDataSnapshot()) }
    }
}
